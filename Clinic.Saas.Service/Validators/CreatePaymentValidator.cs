...
public class CreatePaymentValidator : AbstractValidator<CreatePaymentDto>
{
    public CreatePaymentValidator()
    {
        ...
        RuleFor(x => x.TotalAmount).GreaterThan(0).WithMessage("إجمالي الفاتورة يجب أن يكون أكبر من صفر.");
        ...
        RuleFor(x => x).Must(x => x.DiscountAmount <= x.TotalAmount + x.TaxAmount)
            .WithMessage("قيمة الخصم لا يمكن أن تتجاوز إجمالي الفاتورة.");
        RuleFor(x => x).Must(x => x.PaidAmount <= x.TotalAmount + x.TaxAmount - x.DiscountAmount)
